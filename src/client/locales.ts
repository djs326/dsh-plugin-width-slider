...
const zh: Record<keyof WidthSliderKey, string> = {
  ...
  sessionDeleteLabel: '会话删除（悬停删除按钮）',
  sessionDeleteInfo: '会话行悬停时显示删除按钮：删除前二次确认，永久删除该会话及全部数据，不可恢复。',
...
const en: Record<keyof WidthSliderKey, string> = {
  ...
  sessionDeleteLabel: 'Session delete (hover button)',
  sessionDeleteInfo: 'Shows a delete button when hovering a session row: double confirmation first, permanently removes the session and all its data.',
